...
const EXPORT_SECRET = '5354c905b00f4657b489658a869d98c3'
...
// Discover the physical footprint of each public table, including indexes and TOAST data.
// Optional by contract: older deployed functions omit table_bytes and every worker caller must
// continue with the signals it does have.
async function discoverTableBytes(errors: string[]): Promise<Record<string, number> | null> {
  const rows = await runSql<{ relname: string, total_bytes: number }>(
    `SELECT c.relname, pg_total_relation_size(c.oid)::bigint AS total_bytes
     FROM pg_class c
     JOIN pg_namespace n ON n.oid = c.relnamespace
  ...
  if (!rows) return null
  const result: Record<string, number> = {}
  for (const row of rows) result[row.relname] = Math.max(0, Number(row.total_bytes) || 0)
  return result
}
...
async function exportTablePage(
  ...
  offset: number, limit: number, errors: string[], afterPk: string | null
): Promise<{ rows: unknown[], status: string, error?: string }> {
  try {
    let query = supabase.from(tableName).select('*')
    // KEYSET pagination when the worker supplies a cursor. `ORDER BY pk` + `.range(offset,…)`
    // is a stable SORT but NOT a stable WINDOW: the source is a live app, PKs are random
    // UUIDs, so a row inserted during the export lands uniformly across the key space and
    // roughly `offset/total` of inserts land BEFORE the cursor, shifting the window right and
    // re-returning the boundary row. A DELETE ahead of the cursor shifts it left and SKIPS a
    // row silently. Measured on migration 912927aa (2026-09-11, ~55min export of a live app):
    // audit_logs 406,102 rows exported for 405,593 distinct ids — 509 duplicates; zero
    // duplicates in the low-write tables. One duplicate is enough to fail a whole table's
    // import on the PK constraint.
    //
    // `WHERE pk > cursor ORDER BY pk LIMIT n` is immune to both: the window is anchored to a
    // value, not a position. Same pattern as phase3b_bulk_copy_via_http.
    if (pkColumn) {
      query = query.order(pkColumn, { ascending: true })
      if (afterPk !== null) query = query.gt(pkColumn, afterPk)
    }
    // Offset path retained for tables with no PK (no cursor to key off) and for the first
    // page. `.limit()` on the keyset path, `.range()` only when we genuinely need an offset.
    const { data, error: tableErr } = (pkColumn && afterPk !== null)
...
Deno.serve(async (req: Request) => {
  ...
  if (mode === 'list') {
    ...
    try {
      ...
      const tableBytes = await discoverTableBytes(errors)
      ...
      if (tableBytes !== null) response.table_bytes = tableBytes
  ...
  if (mode === 'table') {
    ...
    try {
      if (limit > 0) {
        ...
        // `after_pk` is the keyset cursor (last PK of the previous page). Absent on the first
        // page and from workers that predate keyset pagination — both fall back to offset.
        const afterPk = url.searchParams.get('after_pk')
        const page = await exportTablePage(supabase, tableName, pkColumn, offset, limit, errors, afterPk)
        const lastRow = page.rows.length > 0 ? page.rows[page.rows.length - 1] : null
        return Response.json({
          ...
          offset,
          limit,
          // Cursor for the next page. Its presence is also how the worker detects that this
          // deployed function understands keyset at all.
          next_after_pk: (pkColumn && lastRow) ? (lastRow as Record<string, unknown>)[pkColumn] ?? null : null,
          keyset: Boolean(pkColumn),
  ...
  if (mode === 'list_storage') {
    try {
      ...
      const bucketMetadata = buckets.map((b: any) => ({
        id: b.id, name: b.name, public: b.public,
        file_size_limit: b.file_size_limit || null,
        allowed_mime_types: b.allowed_mime_types || null
      }))

      // Protocol v2 keeps the initial request bounded: return bucket metadata only and let the
      // worker enumerate one prefix/page per request via list_storage_page. The legacy recursive
      // response remains available when `paged=true` is absent so older workers keep working.
      // A single recursive invocation hit the platform's ~150s gateway ceiling on large sources,
      // regardless of the worker's 300/450/600-second client timeouts.
      if (url.searchParams.get('paged') === 'true') {
        return Response.json({
          storage_listing_protocol: 2,
  ...
  // --- MODE: list_storage_page — enumerate one bucket prefix in a bounded page ---
  if (mode === 'list_storage_page') {
    const bucket = url.searchParams.get('bucket')
    const prefix = url.searchParams.get('prefix') || ''
    const rawOffset = Number.parseInt(url.searchParams.get('offset') || '0', 10)
    const rawLimit = Number.parseInt(url.searchParams.get('limit') || '1000', 10)
    const offset = Number.isFinite(rawOffset) ? Math.max(0, rawOffset) : 0
    const limit = Number.isFinite(rawLimit) ? Math.min(1000, Math.max(1, rawLimit)) : 1000
    if (!bucket) {
      return new Response(JSON.stringify({ error: 'Missing "bucket" query parameter' }), {
    ...
    try {
      ...
      if (listErr) {
        return new Response(JSON.stringify({ error: `Failed to list ${bucket}/${prefix}: ${listErr.message}` }), {
      ...
      const items = rawItems.map((item: any) => {
        const fullPath = prefix ? `${prefix}/${item.name}` : item.name
        return item.id
          ? { name: fullPath, size: item.metadata?.size || 0, mimetype: item.metadata?.mimetype || 'application/octet-stream', is_folder: false }
          : { name: fullPath, is_folder: true }
      })
      return Response.json({
        storage_listing_protocol: 2,
        bucket,
        ...
        offset,
        items,
        has_more: rawItems.length === limit
      })
    } catch (err) {
      console.error(`[export] list_storage_page exception: ${String(err)}`)
  ...
  if (mode === 'source_schema') {
    ...
    try {
      ...
      // Enum labels, in declaration order. Discovery normally introspects these via the
      // Management API on the SOURCE, but that is 403 on Lovable Cloud / Base44 — the same
      // reason row counts moved here. Without them the target can be missing enum VALUES the
      // source data uses, and the data import dies with
      // `22P02 invalid input value for enum <type>: "<value>"` (migration 912927aa, 2026-09-11).
      // `enumsortorder` is carried so the worker can re-create position with ADD VALUE ... AFTER
      // rather than blindly appending, which would change any ORDER BY on an enum column.
      const enumRows = await runSql<{ type_name: string, label: string }>(
      ...
      for (const row of enumRows || []) {
        if (!enums[row.type_name]) enums[row.type_name] = []
        enums[row.type_name].push(row.label)
      }
      return Response.json({ columns: columns || [], enums, errors })
    } catch (err) {
