...
assert(formatSender(null, null) === "eterfaceID <support@verify.eterfaceid.com>", "default sender uses verified domain");
assert(
...
assert(formatSender("  ", "  ") === "eterfaceID <support@verify.eterfaceid.com>", "blank sender falls back");
assert(
  normalizeResendFromAddress("alerts@eterfaceid.com") === "alerts@verify.eterfaceid.com",
  "local part is kept when remapping",
);
assert(
  normalizeResendFromAddress("hello@verify.eterfaceid.com") === "hello@verify.eterfaceid.com",
